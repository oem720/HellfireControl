...
private:
	...
	uint64 m_u32SuccessfulTests = 0u;
public:
	TestBlock() : m_strBlockName("DEFAULT"), m_vTests() {}
	TestBlock(const String& _strName) : m_strBlockName(_strName), m_vTests() {}

	void AddTest(const TestCase& _tcUnitTest);
	void AddTest(const String& _strFuncName, const Function<const bool(float&)>& _funcTest);
	...
	[[nodiscard]] HC_INLINE uint64 GetSuccessfulTestCount() const { return m_u32SuccessfulTests; }
	[[nodiscard]] HC_INLINE uint64 GetTestCount() const { return m_vTests.size(); }
};